...
- Mock server: `server/index.js` allows the deployed production origin (`https://viora-website-wa.web.app`) in CORS, in addition to the Vite dev server, so it can be hosted for the still-unimplemented `/iot-device-summaries` endpoint that the real backend doesn't serve yet.
...
- UI: refreshed favicon and icon sprite artwork.
